...
These examples show you how to download, process, and then visualize Overture data in a Jupyter notebook using [Lonboard](https://developmentseed.org/lonboard/latest/), a Python library for fast, interactive geospatial vector data visualization.
...
The [`from_duckdb()`](https://developmentseed.org/lonboard/latest/api/layers/base-layer/#lonboard.BaseArrowLayer.from_duckdb) function allows us to download Overture data with DuckDB and then pass the resulting table directly into Lonboard.
...
2. Use DuckDB in python to create a table with building geometries and heights:

    ```python
    con = duckdb.connect("demo.dbb") # Create a DB file for persistence
    con.sql("INSTALL SPATIAL; LOAD SPATIAL;")
    con.sql("""
    CREATE OR REPLACE TABLE nyc_buildings AS
        SELECT
            id, IF(height IS NULL,3,height) as height, geometry
        FROM
            read_parquet('s3://overturemaps-us-west-2/release/2024-11-13.0/theme=buildings/type=building/*')
        WHERE
            (bbox.xmin BETWEEN -74.021089 AND -73.966989) AND (bbox.ymin BETWEEN 40.699531  AND 40.757202)
    """)
    ```

3. Pass the table directly into lonboard, including heights for 3d extrusion:

    ```python
    layer = PolygonLayer.from_duckdb(
        con.table("nyc_buildings").select('geometry'),
        con,
        extruded=True,
        get_elevation=con.table("nyc_buildings").arrow()['height'],
        get_fill_color='orangered',
        opacity=0.5,
    )
    m = Map(layer)
    m
    ...
    <!-- ![](/img/examples/nyc_3d_buildings_lonboard.png) -->
    ![](/img/examples/nyc_buildings.gif)
